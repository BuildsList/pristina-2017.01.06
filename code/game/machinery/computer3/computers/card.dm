	...
	proc/scriptblock()
		...
		for(var/job in security_positions)
			...
			if(counter >= 6)
				...
				counter = 0
			jobs_all += "<td height='20' weight='100'><a href='?src=\ref[src];assign=[job]'>[replacetextSS(job, " ", "&nbsp")]</a></td>"
		...
		for(var/job in medical_positions)
			...
			if(counter >= 6)
				jobs_all += "</tr><tr height='20'><td></td><td></td>"
				counter = 0
			jobs_all += "<td weight='100'><a href='?src=\ref[src];assign=[job]'>[replacetextSS(job, " ", "&nbsp")]</a></td>"
		...
		for(var/i = 1; i <= 7; i++)
			...
			for(var/A in get_region_accesses(i))
				if(A in writer.access)
					accesses += topic_link(src,"access=[A]","<font color='red'>[replacetextSS(get_access_desc(A), " ", "&nbsp")]</font>") + " "
				else
					accesses += topic_link(src,"access=[A]",replacetextSS(get_access_desc(A), " ", "&nbsp")) + " "
				accesses += "<br>"
			accesses += "</td>"
		accesses += "</tr></table>"
		return accesses
...
/datum/file/program/card_comp/centcom
	...
	accessblock()
		...
		for(var/A in get_all_centcom_access())
			if(A in writer.access)
				accesses += topic_link(src,"access=[A]","<font color='red'>[replacetextSS(get_centcom_access_desc(A), " ", "&nbsp")]</font>") + " "
			else
				accesses += topic_link(src,"access=[A]",replacetextSS(get_centcom_access_desc(A), " ", "&nbsp")) + " "
		return accesses
