...
//Regular jobs
	//Command (Blue)
		...
		for(var/jobPos in command_positions)
			...
			if(jobban_isbanned(M, job.title))
				jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=[job.title];jobban4=\ref[M]'><font color=red>[replacetextSS(job.title, " ", "&nbsp")]</font></a></td>"
				counter++
			else
				jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=[job.title];jobban4=\ref[M]'>[replacetextSS(job.title, " ", "&nbsp")]</a></td>"
				counter++
	...
	//Antagonist (Orange)
		...
		for(var/antag_type in all_antag_types)
			...
			if(jobban_isbanned(M, "[antag.bantype]") || isbanned_dept)
				jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=[antag.bantype];jobban4=\ref[M]'><font color=red>[replacetextSS("[antag.role_text]", " ", "&nbsp")]</font></a></td>"
			else
				jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=[antag.bantype];jobban4=\ref[M]'>[replacetextSS("[antag.role_text]", " ", "&nbsp")]</a></td>"
